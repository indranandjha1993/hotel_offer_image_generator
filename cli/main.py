...
def validate_word_limit(value):
    try:
        ...
        console.print(
            "[bold red]Invalid word limit. Please enter a number between 1 and 100.[/bold red]"
        )
        return None
...
def validate_opacity(value):
    try:
        ...
        console.print(
            "[bold red]Invalid opacity. Please enter a number between 0 and 1.[/bold red]"
        )
        return None
...
@click.option(
    "--prompt",
    prompt="Enter the hotel offer prompt",
    default="Luxurious beach resort",
    help="Hotel offer prompt",
)
@click.option(
    "--word-limit",
    prompt="Enter word limit",
    default=Config.DEFAULT_WORD_LIMIT,
    ...
    try:
        ...
        while True:
            ...
            if word_limit is not None:
                break
            word_limit = click.prompt(
                "Enter word limit", default=Config.DEFAULT_WORD_LIMIT
            )

        font = prompt_with_choices(
            "Choose a font", os.listdir(Config.FONTS_FOLDER), Config.DEFAULT_FONT
        )
        font_size = int(
            prompt_with_choices(
                "Choose font size",
                [str(size) for size in Config.FONT_SIZES],
                str(Config.DEFAULT_FONT_SIZE),
            )
        )
        position = prompt_with_choices(
            "Choose text position",
            list(Config.TEXT_POSITIONS.keys()),
            Config.DEFAULT_TEXT_POSITION,
        )
        text_color = prompt_with_choices(
            "Choose text color",
            list(Config.TEXT_COLORS.keys()),
            Config.DEFAULT_TEXT_COLOR,
        )
        bg_color = prompt_with_choices(
            "Choose background color",
            list(Config.BACKGROUND_COLORS.keys()),
        ...
        with console.status("[bold green]Initializing services...") as status:
            ...
            service = OfferGeneratorService(
                text_generator, image_generator, image_processor
            )
        ...
        with Progress(
            ...
            *Progress.get_default_columns(),
            ...
            image_task = progress.add_task(
                "[green]Generating initial image...", total=None
            )
            overlay_task = progress.add_task(
                "[blue]Applying text overlay...", total=None
            )
        ...
        console.print(
            Panel(
                f"[bold cyan]Generated offer text:[/bold cyan] {offer_text}",
                expand=False,
            )
        )

        filename = image_processor.get_timestamp_filename(
            offer_text.lower().strip().replace(" ", "_")
        )
