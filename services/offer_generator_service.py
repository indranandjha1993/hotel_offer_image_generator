...
class OfferGeneratorService:
    ...
    def generate_offer(self, prompt: str, word_limit: int, style: TextStyle,
                       text_progress: Callable[[float], None] = None,
                       image_progress: Callable[[float], None] = None,
                       overlay_progress: Callable[[float], None] = None) -> Tuple[str, Image.Image, Image.Image]:
        try:
            offer_text = self.text_generator.generate_offer_text(prompt, word_limit, progress_callback=text_progress)
            initial_image = self.image_generator.generate_image(prompt, progress_callback=image_progress)
            final_image = self.image_processor.overlay_text_on_image(
                initial_image.copy(), offer_text,
                style,
